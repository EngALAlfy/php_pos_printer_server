...
class Escpos
{
	...
	public function print_invoice($data)
	{
		...
		$fontPath = dirname(__FILE__) . "/../I18N/Arabic/Examples/GD/arial.ttf";
		...
		if (isset($data->kitchen_name) && !empty($data->kitchen_name) && isset($data->table) && !empty($data->table)) {
			...
			$this->printer->text(rtrim($this->columnify($data->kitchen_name, $this->columnify($data->table_label, $data->table, 25, 25, 0, 0), 50, 50, 0, 0)));
